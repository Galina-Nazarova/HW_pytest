...
9. test_decorators тесты для проверки декоратора (записи в консоль и в файл)


## Декорирование:
1. Модуль decorаtors.py содержит декоратор функций из модуля widget.py
...
3. Поскольку исключения формирует декорируемая функция, исключения возвращаются в качестве результата вызова декорируемой функции.

## Считывание данных (CSV и Excel)

В проект добавлен модуль `src/data_load.py` для загрузки финансовых транзакций из excel и csv:
1. Функция read_csv_transactions(file_path) — импортирует транзакции из файлов CSV.
2. Функция read_excel_transactions(file_path) — импортирует транзакции из файлов Excel (`.xlsx`).

## Обновление функционала (Тема 13.2)

В рамках текущего этапа разработки проект был расширен новыми возможностями:
1. **Регулярные выражения (`re`)**: В модуль `src/search_data.py` добавлена функция `process_bank_search`. 
Она позволяет осуществлять поиск транзакций по ключевым словам в поле описания (`description`).
2. **Анализ и подсчет категорий (`Counter`)**: Добавлена функция `process_bank_operations`, 
которая использует класс `collections.Counter` для мгновенного аналитического подсчета частоты 
встречаемости заданных типов банковских операций.
3. **Интерактивный интерфейс пользователя**: Корневой модуль `main.py` теперь представляет 
собой полноценную консольную утилиту, которая пошагово опрашивает пользователя, проводит 
валидацию ввода статусов транзакций, фильтрует данные из разных источников (JSON, CSV, XLSX) 
и выводит итоговую замаскированную выборку в строгом соответствии с техническим заданием.
